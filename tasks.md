...
- [x] 🔁 블록 템플릿 → 캘린더 드래그 앤 드롭 배치 (템플릿을 끌어다 놓으면 인스턴스 생성) — verify: 같은 템플릿으로 서로 다른 날짜에 두 번 이상 배치 가능한지 확인
...
- [x] 🔁 블록 템플릿 생성/관리 화면 (제목, 색상, 태그만 — 날짜/시간 없음) — verify: 생성한 템플릿이 팔레트에 표시되는지 확인
