...
- [x] 타이머 상태머신 구현 (실행중 / 자동일시정지 / 수동정지 3상태) — verify: 상태 전이 단위 테스트, 특히 "수동정지 중 포그라운드 전환은 무반응" 케이스
- [x] `AppLifecycleState` 연동 (resumed/paused → 자동 정지/재시작 트리거) — verify: 백그라운드 전환 시뮬레이션 테스트
- [x] 방치 알림 트리거 로직 (수동정지 N분 경과 시 알림) — verify: 타이머 기반 알림 발송 테스트
- [x] 반복 인스턴스 생성 로직 (`RecurrenceRule` → 실제 `Block` row 생성, 향후 N일치) — verify: 매주 반복 규칙으로 4주치 인스턴스 생성 테스트
- [x] 타임블록 중첩 깊이 검증 로직 (1단계 초과 방지) — verify: 2단계 중첩 시도 시 거부 테스트
- [x] 시간 겹침 검사 로직 — verify: 겹치는 두 블록 생성 시 경고 반환 테스트
- [x] 목표 대비 실제 시간 비율 계산 로직 (블록 시간 vs `TimerSessions` 합계) — verify: 샘플 데이터 비율 계산 테스트
- [x] 체크리스트 달성률 계산 로직 (오늘 기준) — verify: 샘플 데이터 비율 계산 테스트
- [x] 잔디(출석) 데이터 계산 로직 (날짜별 완료 데이터 집계) — verify: 특정 날짜 집계 결과 테스트
- [x] 마감 작업 상태 계산 로직 (진행중 / 마감지남) — verify: 마감일 경과 케이스 테스트

---

## Phase 3. 기능별 UI

> ⚠️ **리팩토링 필요**: 이미 완료된 "블록 생성/편집 화면"·"드래그 앤 드롭 배치"는 템플릿/인스턴스 미분리 버전으로 구현됨. 아래 항목 중 🔁 표시는 기존 구현을 고쳐야 함.
- [x] 계획표 캘린더 뷰 (일/주/달/년 전환, 블록 배치) — verify: 각 기간 단위 전환 동작 확인
- [x] 🔁 블록 템플릿 → 캘린더 드래그 앤 드롭 배치 (시간 피커 없이 즉시 1시간 인스턴스 생성, 15분 스냅) — verify: 같은 템플릿으로 서로 다른 날짜에 두 번 이상 배치 가능한지 확인
- [x] 블록 드래그 앤 드롭 배치 — verify: 드래그로 시간대 변경 동작 확인
- [x] 블록 시각적 크기 비례 렌더링 (시간 길이 ↔ 크기) — verify: 1시간/2시간 블록 크기 비교 확인
- [x] 블록 겹침 경고 UI — verify: 겹치는 블록 생성 시 경고 표시 확인
- [x] 🔁 블록 템플릿 생성/관리 화면 (제목, 색상, 태그만 — 날짜/시간 없음) + DraggableScrollableSheet 팔레트 (축소↔확장 스냅) — verify: 생성한 템플릿이 팔레트에 표시되는지, 위로 드래그 시 그리드 펼쳐지는지 확인
- [x] 블록 생성/편집 화면 (제목, 색상, 태그) — verify: 생성 후 캘린더 반영 확인
- [x] 자식 블록(타임블록/체크리스트) 추가 UI — verify: 부모 블록에 자식 추가 후 표시 확인
- [x] 습관 스태킹 UI (블록 체인 연결) — verify: 연결된 블록 순서 표시 확인
- [x] 타이머 화면 (시작/정지 버튼, 상태 표시) — verify: 3상태 전환이 UI에 반영되는지 확인
- [x] 뽀모도로 모드 설정 화면 (온/오프, 시간 설정) — verify: 설정값 저장/적용 확인
- [x] 출석 잔디 뷰 (달력형 그리드, 클릭 시 상세) — verify: 잔디 클릭 시 해당일 활동 표시 확인
- [x] 목표/통계 대시보드 (달성률 시각화) — verify: 퍼센트 그래프 렌더링 확인
- [x] "아직 끝내지 못한 일" 섹션 UI — verify: 미완료 항목 목록 표시, 완료/삭제 동작 확인
- [x] 반복 설정 UI — verify: 반복 규칙 생성 후 캘린더 반영 확인
- [x] 템플릿 저장/불러오기 UI (하루/주 단위) — verify: 템플릿 적용 시 블록 일괄 생성 확인
- [x] 회고 메모 입력 UI (블록 단위) — verify: 메모 저장/조회 확인
- [x] ~~오늘의 기분 이모지 선택 UI~~ — **제거됨** (spec 제외)
- [x] 블록 인스턴스 개별 삭제 — verify: 편집 화면 삭제 버튼으로 해당 row만 삭제, 같은 템플릿/반복 규칙의 다른 블록 미영향 확인
